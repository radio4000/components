...
export default class R4Search extends LitElement {
	...
	async onInput(event) {
		...
		console.log('@todo searching for', value)
		const { data, error } = await sdk.supabase.from('channels').select().textSearch('fts', `'${value}'`)
	...
	render() {
		return html`
			...
			<r4-search-results .items="${this.items}"></r4-search-results>
		`
	}
